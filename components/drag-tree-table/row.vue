		...
		>
			...
			>
				...
				<span v-else-if="subItem.type === 'action'">
					<a
						class="action-item"
						v-for="(acItem, acIndex) in fliterColActions(
							subItem.actions,
							model
						)"
						:key="acIndex"
					...
					>
						<i :class="acItem.icon"></i>
						<span
							v-if="acItem.formatter"
							v-html="acItem.formatter(model)"
						></span>
						<span v-else v-html="acItem.text"></span>
					</a>
				</span>
...
export default {
	...
	methods: {
		...
		fliterColActions(actions, model) {
			return actions.filter(
				action =>
					!action.condition || (action.condition && action.condition(model))
			);
		},
	},
};
